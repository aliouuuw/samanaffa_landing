...
import { personas, objectives, Objective, Persona } from "../data";
...
export const SavingsPlanner: React.FC<SavingsPlannerProps> = ({
  ...
  // --- STATE MANAGEMENT ---
  const [simulationMode, setSimulationMode] = useState<"objective" | "persona">("objective");
  const [selectedObjective, setSelectedObjective] = useState<number>(1);
  const [selectedPersona, setSelectedPersona] = useState<string>("");
  const [duree, setDuree] = useState(objectives[0]?.duree || 12);
  const [mensualite, setMensualite] = useState(objectives[0]?.mensualite || 25000);
  ...
  useEffect(() => {
    if (simulationMode === 'persona' && selectedPersonaData) {
      ...
      setSelectedObjective(0); // Reset objective
    }
  ...
  return (
    <section id="savings-planner" className="w-full pt-12 lg:pt-[76px] pb-8 lg:pb-12 bg-white">
                    ...
                    <div className="flex justify-center mb-6">
                        <div className="flex items-center bg-[#F2F8F4] rounded-full p-1">
                            <button 
                                ...
                                className={`px-6 py-2 text-sm lg:text-base font-semibold rounded-full transition-all duration-300 ${simulationMode === 'objective' ? 'bg-gradient-to-br from-[#e8f5e8] to-[#d4f4d4] text-[#435933] shadow-sm' : 'text-gray-500 hover:text-[#435933] bg-transparent'}`}
                            >
                                Par Objectif
                            </button>
                            <button 
                                onClick={() => handleModeChange('persona')}
                                className={`px-6 py-2 text-sm lg:text-base font-semibold rounded-full transition-all duration-300 ${simulationMode === 'persona' ? 'bg-gradient-to-br from-[#e8f5e8] to-[#d4f4d4] text-[#435933] shadow-sm' : 'text-gray-500 hover:text-[#435933] bg-transparent'}`}
                            >
                                Par Profil
                            </button>
                        </div>
